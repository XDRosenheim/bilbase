...
public partial class Spark : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        SqlCarSelect CarSelect = new SqlCarSelect("Spark"); //TODO
        ...
        SqlRowCount rowCounter = new SqlRowCount("comment_Chevrolet_Spark");
        if (rowCounter.Rows == 0) { }
        else
        {
            for (int i = 1; i <= rowCounter.Rows; i++)
            {
                SqlCommentSelectString Comments = new SqlCommentSelectString("SELECT * FROM comment_Chevrolet_Spark WHERE ID=" + i);
    ...
    protected void commentBtn_Click(object sender, EventArgs e)
    {
        if (TextBox.Text != "")
        {
            SqlRunCommandString submitComment = new SqlRunCommandString("INSERT INTO comment_Chevrolet_Spark VALUES('" + TextBox.Text + "','" + Session["UserId"].ToString() + "',1)");
        }
        else
        {
            // Do nothing.
