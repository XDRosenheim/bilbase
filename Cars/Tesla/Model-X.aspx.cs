...
public partial class Model_X : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        SqlRowCount rowCounter = new SqlRowCount("comment_Tesla_Model_X");
        if (rowCounter.Rows == 0) { }
        else
        {
            for (int i = 1; i <= rowCounter.Rows; i++)
            {
                SqlCommentSelectString Comments = new SqlCommentSelectString("SELECT * FROM comment_Tesla_Model_X WHERE ID=" + i);
